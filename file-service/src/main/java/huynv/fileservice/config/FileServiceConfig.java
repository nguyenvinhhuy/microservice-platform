...
/**
 * Registers shared infrastructure beans required for file-service serialization, schema validation, idempotency, and transactional outbox usage.
 */
@Configuration
public class FileServiceConfig {
    ...
    /**
     * Adapts file-service outbox settings into the shared NotificationProperties contract required by shared outbox infrastructure.
     *
     * @param properties File-service properties containing DLQ and retry settings.
     * @return Returns a NotificationProperties instance populated with file-service outbox settings.
     */
    @Bean
    public NotificationProperties notificationProperties(FileServiceProperties properties) {
    ...
    /**
     * Creates an event factory that stamps file-service as the publishing source.
     *
     * @param tracerProvider Optional Micrometer tracer provider used to propagate active trace identifiers.
     * @return Returns an event factory configured for file-service envelopes.
     */
    @Bean
    public EventFactory eventFactory(ObjectProvider<Tracer> tracerProvider) {
        ...
        return new EventFactory("file-service", () -> {
    ...
    /**
     * Creates the shared JSON Schema validation service used before publishing and after consuming events.
     *
    ...
    /**
     * Creates the shared transactional outbox service backed by the file-service database.
     *
    ...
    /**
     * Creates a JDBC-backed consumer idempotency service for scan-result processing.
     *
     * @param dataSource Primary datasource used to access the processed_events table.
     * @return Returns an idempotency service backed by the processed_events table.
     */
    @Bean
    public IdempotencyService idempotencyService(DataSource dataSource) {
        Objects.requireNonNull(dataSource, "dataSource");
        return new JdbcIdempotencyService(dataSource, "file-service");
    }
}
