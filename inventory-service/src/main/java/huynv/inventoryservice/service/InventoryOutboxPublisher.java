...
import huynv.inventoryservice.config.KafkaTopicConfig;
...
/**
 * Publishes committed inventory outbox rows to Kafka with at-least-once delivery semantics.
 */
@Component
@Slf4j
public class InventoryOutboxPublisher {

    private static final io.opentelemetry.api.trace.Tracer tracer = GlobalOpenTelemetry.getTracer("inventory-service");

    private final InventoryOutboxTransactionalService outboxTransactionalService;
    ...
    @Value("${feature.inventory.outbox.enabled:true}")
    private boolean outboxEnabled;

    @Value("${inventory.outbox.publisher-delay-ms:2000}")
    ...
    /**
     * Creates an outbox publisher for inventory events.
     *
    ...
    public InventoryOutboxPublisher(
            InventoryOutboxTransactionalService outboxTransactionalService,
            @Qualifier("inventoryOutboxKafkaTemplate") KafkaTemplate<String, String> kafkaTemplate,
        ...
        this.outboxPublishTimer = Timer.builder("outbox_publish_latency").tag("service", "inventory-service").register(meterRegistry);
        this.outboxRetryCounter = meterRegistry.counter("outbox_retry_total", "service", "inventory-service");
    ...
    /**
     * Publishes due outbox events in deterministic batches and updates row state after publish attempts.
     *
     * @return Publishes committed rows and persists publish status for retries.
     */
    @Scheduled(fixedDelayString = "${inventory.outbox.publisher-delay-ms:2000}")
    @SchedulerLock(name = "inventory-service-outbox-publisher", lockAtMostFor = "PT30S", lockAtLeastFor = "PT2S")
    public void publishDueOutbox() {
        if (!outboxEnabled) {
            log.warn("Outbox publisher skipped because feature.inventory.outbox.enabled=false");
            return;
        }
    ...
    /**
     * Publishes a single outbox row payload to Kafka and finalizes row status.
     *
    ...
    private void publishSingle(OutboxEvent row) {
        ...
        try {
            ...
            ProducerRecord<String, String> record = new ProducerRecord<>(
                    KafkaTopicConfig.INVENTORY_EVENTS_TOPIC,
                    row.getPartitionKey(),
                    row.getPayload()
            );
            ...
            log.warn("Inventory outbox publish failed id={} eventId={} type={} reason={}", row.getId(), row.getEventId(), row.getEventType(), ex.getMessage());
        } finally {
